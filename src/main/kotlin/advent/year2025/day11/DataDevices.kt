...
/**
 * --- Day 11: Reactor ---
 * You hear some loud beeping coming from a hatch in the floor of the factory, so you decide to check it out. Inside,
 * you find several large electrical conduits and a ladder.
 ...
 * but we aren't having any luck getting the reactor to communicate with it!" You glance around the room and see a
 * tangle of cables and devices running from the server rack to the reactor. She rushes off, returning a moment later
 ...
 * iii: out
 *
 * Each line gives the name of a device followed by a list of the devices to which its outputs are attached. So, bbb:
 * ddd eee means that device bbb has two outputs, one leading to device ddd and the other leading to device eee.
 ...
 * After dividing up the work, the Elves would like you to focus on the devices starting with the one next to you (an
 * Elf hastily attaches a label which just says you) and ending with the main output to the reactor (which is the
 * device with the label out).
 ...
 * problematic data path passes through both dac (a digital-to-analog converter) and fft (a device which performs a
...
class DataDevices(
  ...
  private val inputsByDevice: Map<String, List<String>>
) {
  constructor(input: String) : this(input.trim().lines().map(DataDevice::of))
  constructor(devices: List<DataDevice>) : this(
    devices.flatMap { it.to.map { output -> it.from to output } }
      .groupBy(keySelector = { it.second }, valueTransform = { it.first })
  )


  fun countPaths(origin: String, target: String): Long = pathCountCache[origin to target]
  private val pathCountCache = loadingCache(::calculatePathCountTo)
  private fun calculatePathCountTo(originAndTarget: Pair<String, String>): Long {
    val (origin, target) = originAndTarget
    if (target == origin) return 1
    val inputDevices = inputsByDevice[target] ?: return 0
...
fun main() {
  ...
  println(devices.countPaths(origin = "you", target = "out"))
  println(devices.countServerPaths())
}