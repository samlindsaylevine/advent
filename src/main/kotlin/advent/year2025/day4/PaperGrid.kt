 ...
 * You ride the escalator down to the printing department. They're clearly getting ready for Christmas; they have lots
 * of large rolls of paper everywhere, and there's even a massive printer in the corner (to handle the really big print
 * jobs).
 ...
 * forklifts might be able to access more rolls of paper, which they might also be able to remove. How many total rolls
 * of paper could the Elves remove if they keep repeating this process?
 ...
 *
 */
class PaperGrid(val rolls: Set<Point>) {
  constructor(input: String) : this(
    input.trim()
      .lines()
      .flatMapIndexed { y, line ->
        (0 until line.length).filter { line[it] == '@' }.map { Point(it, y) }
      }
      .toSet()
  )

  fun countAccessible() = rolls.count { it.isAccessible() }
  ...
  fun totalRemovable(): Int {
    ...
    else 1 + PaperGrid(this.rolls - firstAccessible).totalRemovable()
  }
}

fun main() {
  val grid = PaperGrid(readInput())

  println(grid.countAccessible())
  println(grid.totalRemovable())
}