 ...
 * checkpoint. When you get to the main elevators, however, you discover that each one has a red light above it:
 * they're all offline.
 ...
 * are each labeled with their joltage rating, a value from 1 to 9. You make a note of their joltage ratings (your
 ...
 * In 811111111111119, you can make the largest joltage possible by turning on the batteries labeled 8 and 9, producing
 * 89 jolts.
 ...
 * In 234234234234278, the largest joltage can be found by turning on everything except a 2 battery, a 3 battery, and
...
class BatteryBanks(val banks: List<BatteryBank>) {
  ...
  fun totalOutputJoltage(numDigits: Int = 2) = banks.sumOf { it.largestJoltage(numDigits) }
}
...
class BatteryBank(val batteries: List<Int>) {
  ...
  fun largestJoltage(numDigits: Int) = batteries.largestJoltage(numDigits)

  private fun List<Int>.largestJoltage(numDigits: Int): Long = when {
    numDigits < 1 -> throw IllegalArgumentException("Can only calculate joltage with a positive number of digits")
    numDigits == 1 -> this.max().toLong()
    else -> {
      val leadingDigit = this.dropLast(numDigits - 1).max()
      val leadingDigitPlace = this.indexOf(leadingDigit)
      val remaining = this.drop(leadingDigitPlace + 1)
      (leadingDigit.toString() + remaining.largestJoltage(numDigits - 1).toString()).toLong()
    }
  }
}
...
fun main() {
  ...
  println(banks.totalOutputJoltage(numDigits = 12))
}