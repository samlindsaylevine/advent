 ...
 * their usual Christmas emergency. For example, they now know that the North Pole decorations need to be finished soon
...
class SafeDialSequence(val steps: List<Int>) {
  ...
  data class SafeDialState(val currentPosition: Int, val timesClickedOntoOrPastZero: Int) {
    fun dial(stepValue: Int): SafeDialState {
      val nextTarget = currentPosition + stepValue
      val nextPosition = Math.floorMod(nextTarget, DIAL_SIZE)
      // This is kind of dorky, looking at every actual number we step through, instead of doing any smart
      // arithmetic, but it does exactly meet the problem statement.
      val allTargets = if (stepValue < 0) (currentPosition downTo nextTarget) else (currentPosition..nextTarget)
      val zeroes = allTargets.drop(1).count { it % DIAL_SIZE == 0 }
      return SafeDialState(nextPosition, this.timesClickedOntoOrPastZero + zeroes)
    }
  }

  fun states(): Sequence<SafeDialState> = steps.asSequence()
    .runningFold(
      SafeDialState(currentPosition = INITIAL_VALUE, timesClickedOntoOrPastZero = 0),
      SafeDialState::dial
    )

  fun password() = states().count { it.currentPosition == 0 }
  fun clickPassword() = states().last().timesClickedOntoOrPastZero
}
...
fun main() {
  ...
  println(sequence.clickPassword())
}
