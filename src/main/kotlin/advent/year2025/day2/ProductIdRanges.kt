...
class ProductIdRanges(val ranges: List<ProductIdRange>) {
  ...
  fun invalidIdSum(test: (String) -> Boolean = String::isSequenceRepeatedTwice) = ranges.sumOf { it.invalidIdSum(test) }
}
...
class ProductIdRange(val first: Long, val second: Long) {
  ...
  fun invalidIdSum(test: (String) -> Boolean) = (first..second).filter { test(it.toString()) }.sum()
}
...
fun String.isSequenceRepeated(): Boolean {
  val sequenceLengths = this.length.factors().dropLast(1)
  return sequenceLengths.any { length ->
    val groups = this.chunked(length)
...
fun main() {
  ...
  println(ranges.invalidIdSum(String::isSequenceRepeated))
}