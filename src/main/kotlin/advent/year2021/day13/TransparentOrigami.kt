...
class TransparentOrigami(
  ...
  private val folds: List<OrigamiFold>
) {
  ...
  override fun toString() = points.visualized('#')

  fun foldedOnce(): TransparentOrigami {
    val fold = folds.first()

    return TransparentOrigami(
      points.map { it.reflectedAcross(fold) }.toSet(),
      folds.drop(1)
    )
  }

  fun folded(): TransparentOrigami = if (this.folds.isEmpty()) this else (this.foldedOnce().folded())
}

// When we reflect across a line at, say, X = 10, a point at X = 9 stays where it is. A point at X = 12 goes to
// X = 10 + (10 - 12).
private fun Point.reflectedAcross(fold: OrigamiFold): Point = when (fold.direction) {
  FoldDirection.X -> Point(if (this.x < fold.number) this.x else 2 * fold.number - this.x, this.y)
  FoldDirection.Y -> Point(this.x, if (this.y < fold.number) this.y else 2 * fold.number - this.y)
}

data class OrigamiFold(val number: Int, val direction: FoldDirection) {
  constructor(input: String) : this(
    input.substringAfter("=").toInt(),
    input.substringAfter("fold along ")
      .substringBefore("=")
      .let(FoldDirection::from)
  )
}
...
enum class FoldDirection {
  ...
  companion object {
    fun from(input: String) = when (input) {
      "x" -> X
      "y" -> Y
      else -> throw IllegalArgumentException("Unknown fold direction $input")
    }
  }
}

fun main() {
  val origami = File("src/main/kotlin/advent/year2021/day13/input.txt")
    .readText()
    .trim()
    .let(::TransparentOrigami)

  println(origami.foldedOnce().points.size)
  println(origami.folded())
}