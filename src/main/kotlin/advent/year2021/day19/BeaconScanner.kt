 ...
 * difficult to navigate in the pitch black open waters of the ocean trench, but if you can build a map of the trench
 ...
 * position determined relative to the scanner. However, scanners cannot detect other scanners. The submarine has
 ...
 * 1501,0,-500, the scanner could report that the first beacon is at -1000,1000,-1000 (relative to the scanner) but
 ...
 *
 * --- Part Two ---
 * Sometimes, it's a good idea to appreciate just how big the ocean is. Using the Manhattan distance, how far apart do
 * the scanners get?
...
class BeaconScanners(val scanners: List<BeaconScanner>) {
   ...
   * coordinate system.
   */
  fun beacons(): Set<Point3D> = pointsFromScanners { it.points.toSet() }

  /**
   * Finds the locations of all the scanners in the first scanner's coordinate system.
   */
  private fun scannerLocations(): Set<Point3D> = pointsFromScanners { setOf(Point3D(0, 0, 0)) }

  /**
   * Gather the points from each scanner and map them all the way down to the first scanner's coordinate system.
   *
   * @param [points] The points from each scanner, in their own coordinate system, to assemble.
   */
  private fun pointsFromScanners(points: (BeaconScanner) -> Set<Point3D>): Set<Point3D> =
    mappings.keys.fold(emptySet()) { acc, scanner ->
      acc + mapPoints(mappings, points(scanner), scanner, scanners.first())
    }
