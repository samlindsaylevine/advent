...
class AntennaMap(val width: Int, val height: Int, val antennas: List<Antenna>) {
        ...
        fun of(input: String): AntennaMap {
            ...
            val antennas = lines.flatMapIndexed { y, line ->
                line.mapIndexedNotNull { x, c ->
                    if (c == '.') null else Antenna(c, Point(x, y))
                }
            }
            return AntennaMap(width, height, antennas)
        }
    }
    ...
    /**
     * Returns all the antinodes caused by the interaction of two antennas, along the direction from first to second.
...
fun main() {
    val map = AntennaMap.of(readInput())

    println(map.antinodes().size)
    println(map.antinodes(harmonics = true).size)
}