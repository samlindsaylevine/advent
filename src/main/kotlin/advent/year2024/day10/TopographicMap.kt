 ...
 *
 */
class TopographicMap(private val heights: Map<Point, Int>) {
    companion object {
    ...
    // Use (re-entrant) caches so that we don't have to calculate more than once from a particular point.
    ...
    fun trailheadScore(): Int = startingPoints().sumOf { endpointsReachableCache[it].size }

    fun trailheadRating(): Int = startingPoints().sumOf { trailCountCache[it] }
}

fun main() {
    val map = TopographicMap(readInput())

    println(map.trailheadScore())
    println(map.trailheadRating())
}