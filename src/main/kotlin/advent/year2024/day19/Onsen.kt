 ...
 * (w), blue (u), black (b), red (r), or green (g). So, a towel with the pattern ggr would have a green stripe, a green
 ...
 *
 */
class Onsen(private val towels: Set<String>, private val designs: List<String>) {
    ...
    private fun String.towelPrefixes() = (0..maxTowelLength.coerceAtMost(this.length))
        .map { this.substring(0, it) }
        .filter { it in towels }

    private val countPossibilitiesCache = loadingCache(::countPossibilities)
    private fun countPossibilities(target: String): Long {
        if (target.isEmpty()) return 1
        val towels = target.towelPrefixes()
        return towels.sumOf {
            countPossibilitiesCache[target.drop(it.length)]
        }
    }

    fun possibleDesignCount() = designs.count { countPossibilitiesCache[it] > 0 }
    fun possiblitiesCount() = designs.sumOf { countPossibilitiesCache[it] }
}
...
fun main() {
    ...
    println(onsen.possiblitiesCount())
}