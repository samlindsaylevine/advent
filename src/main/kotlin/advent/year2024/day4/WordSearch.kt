 ...
 *
 */
class WordSearch(val letters: Map<Point, Char>) {
    constructor(input: String) : this(
        input.lines().flatMapIndexed { y: Int, line: String ->
            line.mapIndexed { x, c -> Point(x, y) to c }
        }.toMap()
    )

    fun count(phrase: String): Int {
        val directions = Point(0, 0).eightNeighbors
        // The lists of offsets in each of the eight direction that we will check from each origin point.
        val paths = directions.map { direction -> phrase.indices.map { it * direction } }
        return letters.keys.sumOf { origin ->
            val points = paths.map { path -> path.map { it + origin } }
            points.count { phraseAt(it) == phrase }
        }
    }

    private fun phraseAt(points: Iterable<Point>) =
        points.mapNotNull { letters[it] }.joinToString(separator = "")

    fun countCrosses(): Int {
        val aPoints = letters.entries.filter { it.value == 'A' }.map { it.key }
        return aPoints.count { isCross(it) }
    }

    private fun isCross(aPoint: Point): Boolean {
        ...
        val letters = cornerPoints.mapNotNull { letters[it] }
        // We need two M's and two S's to have a cross, and the layout is only correct if the opposite corners are not
        // the same letter.
        return (letters.count { it == 'M' } == 2) &&
                (letters.count { it == 'S' } == 2) &&
                letters.first() != letters.last()
    }
}

fun main() {
    val input = File("src/main/kotlin/advent/year2024/day4/input.txt").readText().trim()
    val wordSearch = WordSearch(input)

    println(wordSearch.count("XMAS"))
    println(wordSearch.countCrosses())
}