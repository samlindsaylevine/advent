...
class MemorySpace(
    val size: Int,
    private val byteAppearanceTimeByLocation: Map<Point, Int>
) {
    constructor(input: String, size: Int = 70) : this(size, input.trim()
        .lines()
        .map { line -> Point(line.substringBefore(",").toInt(), line.substringAfter(",").toInt()) }
        .toAppearanceTimes())

    // Returns null if it is impossible to get to the exit at this time.
    fun minimumStepsAtTime(time: Int): Int? {
        val path = ShortestPathFinder().find(
            Point(0, 0),
            EndState(Point(size, size)),
            Steps { point -> point.adjacentNeighbors.filter { it.isOpenAtTime(time) }.toSet() },
    ...
    /**
     * Time t is after t bytes have fallen.
     */
    private fun Point.isOpenAtTime(time: Int): Boolean {
        if (this.x !in 0..size || this.y !in 0..size) return false
        val byteAppearanceTime = byteAppearanceTimeByLocation[this] ?: return true
        return time <= byteAppearanceTime
    }

    fun draw(time: Int): String = (0..size).joinToString(separator = "\n") { y ->
        (0..size).joinToString(separator = "") { x -> if (Point(x, y).isOpenAtTime(time)) "." else "#" }
    }

    fun firstByteThatBlocksExit(): Point {
        val blockedTime = (0..byteAppearanceTimeByLocation.size).first {
            minimumStepsAtTime(it) == null
        }

        return byteAppearanceTimeByLocation.entries.first { it.value == blockedTime - 1 }.key
    }
}

private fun List<Point>.toAppearanceTimes(): Map<Point, Int> = this.withIndex()
    .reversed()
    .associate { it.value to it.index }

fun main() {
    val space = MemorySpace(readInput())

    println(space.minimumStepsAtTime(1024))
    // This is a little bit slow (~30 seconds) and we could speed it up by doing a binary search
    // instead of a linear one.
    println(space.firstByteThatBlocksExit())
}