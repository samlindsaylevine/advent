 ...
 * mul(X,Y), where X and Y are each 1-3 digit numbers. For instance, mul(44,46) multiplies 44 by 46 to get a result of
 ...
 *
 */
class CorruptedMemory(val text: String) {
    private val multiplicationRegexString = """mul\((\d{1,3}),(\d{1,3})\)"""
    private fun MatchResult.multiplicationResult() = this.groupValues.drop(1).map(String::toInt).reduce(Int::times)

    fun multiplicationSum(): Int {
        val matches = multiplicationRegexString.toRegex().findAll(text)
        return matches.sumOf { it.multiplicationResult() }
    }

    fun conditionalSum(): Int {
        val regex = """$multiplicationRegexString|do\(\)|don't\(\)""".toRegex()
        ...
        matches.forEach { match ->
            when (match.groupValues.first()) {
                "do()" -> enabled = true
                "don't()" -> enabled = false
                else -> if (enabled) sum += match.multiplicationResult()
            }
        }
        return sum
    }
...
fun main() {
    val input = File("src/main/kotlin/advent/year2024/day3/input.txt").readText().trim()
    val memory = CorruptedMemory(input)

    println(memory.multiplicationSum())
    println(memory.conditionalSum())
}