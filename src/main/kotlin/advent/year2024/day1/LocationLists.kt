 ...
 *
 */
class LocationLists(val first: List<Long>, val second: List<Long>) {
    companion object {
        fun of(input: String): LocationLists {
            val pairs = input.trim().lines()
                .map { line -> line.split("\\s+".toRegex()).take(2) }
            return LocationLists(pairs.map { it.first().toLong() }, pairs.map { it.drop(1).first().toLong() })
        }
    }

    fun totalDistance(): Long = first.sorted().zip(second.sorted())
        .sumOf { (it.first - it.second).absoluteValue }

    fun similarityScore(): Long = first.sumOf { value -> value * second.count { it == value } }
}

fun main() {
    val input = File("src/main/kotlin/advent/year2024/day1/input.txt").readText().trim()
    val lists = LocationLists.of(input)

    println(lists.totalDistance())
    println(lists.similarityScore())
}