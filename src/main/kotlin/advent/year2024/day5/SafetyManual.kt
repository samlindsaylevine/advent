 ...
 *
 */
data class SafetyManual(val rules: List<OrderingRule>, val updates: List<Update>) {
    constructor(input: String) : this(
        input.substringBefore("\n\n").lines().map(::OrderingRule),
    ...
    data class Update(val pages: List<Int>) {
        constructor(line: String) : this(line.split(",").map(String::toInt))

        fun middleNumber() = pages[(pages.size - 1) / 2]
        fun satisfies(rule: OrderingRule): Boolean {
            val beforeIndex = pages.indexOf(rule.before)
            val afterIndex = pages.indexOf(rule.after)
            return beforeIndex == -1 || afterIndex == -1 || beforeIndex < afterIndex
        }

        // Since we correct each update by moving its "after" element around, we will sort the rules by their
        // "after" value before applying them to ensure that they are applied in a consistent order.
        fun corrected(rules: List<OrderingRule>): Update = rules.sortedBy { it.after }.fold(this, Update::corrected)

        private fun corrected(rule: OrderingRule) =
            if (this.satisfies(rule)) this else Update(pages.moveToAfter(rule.before, rule.after))
    }

    fun middleNumberSum(): Int = updates.filter { update -> rules.all { update.satisfies(it) } }
        .sumOf { it.middleNumber() }

    fun correctedMiddleNumberSum(): Int = updates.filter { update -> !rules.all { update.satisfies(it) } }
        .map { it.corrected(rules) }
        .sumOf { it.middleNumber() }
}

/**
 * Returns a new list with the element 'after' moved to immediately after the element 'before'.
 * If either element is absent, returns the same list.
 */
fun <E> List<E>.moveToAfter(before: E, after: E): List<E> {
    val beforeIndex = this.indexOf(before)
    val afterIndex = this.indexOf(after)
    if (beforeIndex == -1 || afterIndex == -1 || beforeIndex < afterIndex) return this
    return this.subList(0, afterIndex) +
            this.subList(afterIndex + 1, beforeIndex + 1) +
            listOf(after) +
            this.subList(beforeIndex + 1, this.size)
}

fun main() {
    val manual = SafetyManual(readInput())

    println(manual.middleNumberSum())
    println(manual.correctedMiddleNumberSum())
}