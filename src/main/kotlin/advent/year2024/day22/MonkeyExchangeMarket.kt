...
class MonkeyBuyer(val secret: Long) {
    fun sequence(): Sequence<Long> = generateSequence(secret, ::next)

    private fun next(number: Long): Long {
        var output = number.mix(number * 64).prune()
        output = output.mix(output / 32).prune()
        output = output.mix(output * 2048).prune()
        return output
    }
    ...
    private val sequenceToRevenue: Map<List<Int>, Int> by lazy {
        sequence().drop(1)
            ...
            .windowed(5)
            .toList()
            .reversed() // Reversed so that the first ones are preserved by associate, not the last
            .associate { it.zipWithNext().map { (first, second) -> second - first } to it.last() }
    }

    fun revenue(sequenceOfPriceChanges: List<Int>): Int = sequenceToRevenue[sequenceOfPriceChanges] ?: 0
}

fun main() {
    val market = MonkeyExchangeMarket(readInput())

    println(market.buyers.sumOf { it.secretNumber(2000) })
    println(market.optimalRevenue())
}