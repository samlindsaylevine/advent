 ...
 *
 * So, after blinking once, your five stones would become an arrangement of seven stones engraved with the numbers 1
 ...
 *
 *
 *  @param countsByNumber The number of stones we have with each marking. Notably stones never *merge*, only split,
 *  so we don't need to track order or individual stones, and can instead track our inventory of each distinct number.
 */

class BlinkingStones(val countsByNumber: Map<Long, Long>) {
    ...
    private fun blink(): BlinkingStones {
        val nextCounts = mutableMapOf<Long, Long>()
        countsByNumber.forEach { (number, count) ->
            val nextNumbers = number.next()
            nextNumbers.forEach { nextNumber ->
                nextCounts.merge(nextNumber, count, Long::plus)
            }
        }
        return BlinkingStones(nextCounts)
    }

    fun blink(times: Int) = this.next(times) { it.blink() }
    fun stoneCount() = this.countsByNumber.values.sum()

    private fun Long.next(): List<Long> = when {
        this == 0L -> listOf(1L)
        this.toString().length % 2 == 0 -> this.bisected()
        else -> listOf(this * 2024)
    }

    private fun Long.bisected(): List<Long> {
...
fun main() {
    val stones = BlinkingStones(readInput())

    println(stones.blink(25).stoneCount())
    println(stones.blink(75).stoneCount())
}