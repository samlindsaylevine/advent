...
/**
 * --- Day 22: Crab Combat ---
 * It only takes a few hours of sailing the ocean on a raft for boredom to sink in. Fortunately, you brought a small
 * deck of space cards! You'd like to play a game of Combat, and there's even an opponent available: a small crab that
 ...
 * consists of a series of rounds: both players draw their top card, and the player with the higher-valued card wins
 * the round. The winner keeps both cards, placing them on the bottom of their own deck so that the winner's card is
 * above the other card. If this causes a player to have all of the cards, they win, and the game ends.
 ...
 * of the card multiplied by 1, the second-from-the-bottom card is worth the value of the card multiplied by 2, and so
 * on. With 10 cards, the top card is worth the value on the card multiplied by 10. In this example, the winning
 * player's score is:
 *    3 * 10
 * +  2 *  9
 * + 10 *  8
 ...
 * Captain, you challenge the small crab to a game of Recursive Combat.
 * Recursive Combat still starts by splitting the cards into two decks (you offer to play with the same starting decks
 * as before - it's only fair). Then, the game consists of a series of rounds with a few changes:
 * 
 * Before either player deals a card, if there was a previous round in this game that had exactly the same cards in the
 * same order in the same players' decks, the game instantly ends in a win for player 1. Previous rounds from other
 ...
 * is above the other card). Note that the winner's card might be the lower-valued of the two cards if they won the
 * round due to winning a sub-game. If collecting cards by winning the round causes a player to have all of the cards,
 ...
 * decks to form the sub-game. (For example, if player 1 drew the 3 card, their deck in the sub-game would be copies of
 ...
 * 
 * After the game, the winning player's score is calculated from the cards they have in their original deck using the
 * same rules as regular Combat. In the above game, the winning player's score is 291.
 * Defend your honor as Raft Captain by playing the small crab in a game of Recursive Combat using the same two decks
 * as before. What is the winning player's score?
 * 
 */
