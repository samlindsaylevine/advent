 ...
 * hash these programs are implementing isn't very strong, but you decide to help them anyway. You make a mental note
 * to remind the Elves later not to invent their own cryptographic functions.
 * This hash function simulates tying a knot in a circle of string with 256 marks on it. Based on the input to be
 * hashed, the function repeatedly selects a span of string, brings the ends together, and gives the span a half-twist
 * to reverse the order of the marks within it. After doing this many times, the order of the marks is used to build
 * the resulting hash.
 *   4--5   pinch   4  5           4   1
 *  /    \  5,0,1  / \/ \  twist  / \ / \
 * 3      0  -->  3      0  -->  3   X   0
 ...
 * Then, the current position moves forward by the length, 3, plus the skip size, 0: 2 1 0 [3] 4. Finally, the skip
 * size increases to 1.
 * 
 * 
