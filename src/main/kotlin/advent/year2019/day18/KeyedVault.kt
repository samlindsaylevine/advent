            ...
            input.trim().lines().forEachIndexed { y, line ->
                line.forEachIndexed { x, char ->
                    ...
                    when {
                        ...
                        char.isUpperCase() -> doors[here] = char.lowercaseChar()
                    }
                }
            }
...
private class KeyedVaultGraph(val edges: Map<KeyedVaultNode, Set<VaultEdge>>) {
    ...
    fun shortestPathLength(): Int {
        ...
        val paths = finder.find(
            start = VaultGraphExplorationState(
                edges.keys.filterIsInstance<StartNode>().toSet(),
                emptySet(),
                emptySet()
            ),
    ...
    private fun VaultGraphExplorationState.moving(from: KeyedVaultNode, to: KeyedVaultNode) = when (to) {
        is KeyNode -> VaultGraphExplorationState(
            this.positions - from + to,
            this.keysOwned + to.key,
            this.doorsOpened
        )

        is DoorNode -> VaultGraphExplorationState(
            this.positions - from + to,
            this.keysOwned,
...
private data class VaultGraphExplorationState(
    val positions: Set<KeyedVaultNode>,
