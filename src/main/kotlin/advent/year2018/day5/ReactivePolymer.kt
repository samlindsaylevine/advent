...
data class ReactivePolymer(val formula: String) {
    ...
    private fun Char.isOppositePolarity(other: Char) =
        (this.lowercaseChar() == other || this.uppercaseChar() == other) &&
                this != other
    ...
    private fun unitTypes() = formula.lowercase().toCharArray().toSet()

    fun without(unit: Char) = ReactivePolymer(formula.filter { it != unit && it != unit.uppercaseChar() })
