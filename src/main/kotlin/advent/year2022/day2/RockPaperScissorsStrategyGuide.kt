 ...
 *
 */
class RockPaperScissorsStrategyGuide(val rounds: List<RockPaperScissorsRound>) {
  companion object {
    fun withSecondColumnAsMove(input: String) =
      RockPaperScissorsStrategyGuide(input.lines().map(RockPaperScissorsRound::withSecondColumnAsMove))

    fun withSecondColumnAsOutcome(input: String) =
      RockPaperScissorsStrategyGuide(input.lines().map(RockPaperScissorsRound::withSecondColumnAsOutcome))
  }

  val score = rounds.sumOf { it.score }
}

class RockPaperScissorsRound(opponentMove: RockPaperScissorsMove, myMove: RockPaperScissorsMove) {
  companion object {
    fun withSecondColumnAsMove(input: String) = RockPaperScissorsRound(
      RockPaperScissorsMove.parseOpponentMove(input.substringBefore(" ")),
      RockPaperScissorsMove.parseMyMove(input.substringAfter(" "))
    )

    fun withSecondColumnAsOutcome(input: String): RockPaperScissorsRound {
      val opponentMove = RockPaperScissorsMove.parseOpponentMove(input.substringBefore(" "))
      val myMove = when (input.substringAfter(" ")) {
        "X" -> opponentMove.defeats // Lose
        "Y" -> opponentMove // Draw
        "Z" -> RockPaperScissorsMove.values().first { it.defeats == opponentMove } // Win
        else -> throw IllegalArgumentException()
      }
      return RockPaperScissorsRound(opponentMove, myMove)
    }
  }

  val score = myMove.score + when {
    myMove.defeats == opponentMove -> 6
    myMove == opponentMove -> 3
    else -> 0
  }
}

enum class RockPaperScissorsMove(
  val score: Int
) {
  ROCK(score = 1) {
    override val defeats by lazy { SCISSORS }
  },
  PAPER(score = 2) {
    override val defeats by lazy { ROCK }
  },
  SCISSORS(score = 3) {
    override val defeats by lazy { PAPER }
  };

  abstract val defeats: RockPaperScissorsMove

  companion object {
    fun parseMyMove(input: String) = when (input) {
      "X" -> ROCK
      "Y" -> PAPER
      "Z" -> SCISSORS
    ...
    fun parseOpponentMove(input: String) = when (input) {
      "A" -> ROCK
      "B" -> PAPER
      "C" -> SCISSORS
      else -> throw IllegalArgumentException(input)
...
fun main() {
  val input = File("src/main/kotlin/advent/year2022/day2/input.txt").readText().trim()

  val guide = RockPaperScissorsStrategyGuide.withSecondColumnAsMove(input)
  println(guide.score)

  val decryptedGuide = RockPaperScissorsStrategyGuide.withSecondColumnAsOutcome(input)
  println(decryptedGuide.score)
}