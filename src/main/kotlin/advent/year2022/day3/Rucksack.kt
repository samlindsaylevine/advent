...
import advent.year2022.day3.Rucksack.Companion.priority
import java.io.File
...
class Rucksack(
  ...
  companion object {
    ...
    fun priority(item: Char) = priorities[item] ?: throw IllegalArgumentException("Unrecognized item $item")
  }

  private val firstCompartment = this.items.firstHalf().toSet()
  private val secondCompartment = this.items.secondHalf().toSet()

  val itemInBoth: Char = firstCompartment.intersect(secondCompartment).first()
  val priority: Int = priority(itemInBoth)
}

private fun String.firstHalf() = this.slice(0 until this.length / 2)
private fun String.secondHalf() = this.slice(this.length / 2 until this.length)
...
fun main() {
  ...
  println(rucksacks.totalPriority())
  println(rucksacks.badgePriority())
}