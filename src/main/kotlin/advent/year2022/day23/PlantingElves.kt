...
import advent.year2022.day23.AdjacentPoint.E
import advent.year2022.day23.AdjacentPoint.N
import advent.year2022.day23.AdjacentPoint.NE
import advent.year2022.day23.AdjacentPoint.NW
import advent.year2022.day23.AdjacentPoint.S
import advent.year2022.day23.AdjacentPoint.SE
import advent.year2022.day23.AdjacentPoint.SW
import advent.year2022.day23.AdjacentPoint.W
import java.io.File
...
class PlantingElves(
  val elves: Set<Point>,
  private val roundNumber: Int = 0
) {
  constructor(input: String) : this(input.lines().flatMapIndexed { y, line ->
    line.mapIndexedNotNull { x, c -> if (c == '#') Point(x, -y) else null }
  }.toSet())

  fun after(rounds: Int) = sequence().drop(rounds).first()

  fun firstNoMoveRound() = sequence()
    .zipWithNext()
    .first { it.first.elves == it.second.elves }
    .second
    .roundNumber

  fun sequence() = generateSequence(this) { it.next() }

  fun next(): PlantingElves {
    val elvesWithProposals = elves.map { it to ElfMovementRule.proposal(it, elves, roundNumber) }
    val proposalCounts = elvesWithProposals.groupingBy { it.second }
      .eachCount()

    val targetElves = elvesWithProposals.map { (elf, proposal) ->
      if (proposalCounts[proposal] == 1) proposal else elf
    }.toSet()

    return PlantingElves(targetElves, roundNumber + 1)
  }

  fun emptyGround(): Int {
    val deltaX = elves.maxOf { it.x } - elves.minOf { it.x }
    val deltaY = elves.maxOf { it.y } - elves.minOf { it.y }
    return (deltaX + 1) * (deltaY + 1) - elves.size
  }
}

enum class ElfMovementRule(
  val consideredPositions: List<Point>,
  val direction: Direction
) {
  NORTH(listOf(NW, N, NE), Direction.N),
  SOUTH(listOf(S, SE, SW), Direction.S),
  WEST(listOf(W, NW, SW), Direction.W),
  EAST(listOf(E, NE, SE), Direction.E);

  companion object {
    fun proposal(current: Point, elves: Set<Point>, roundNumber: Int): Point {
      if (current.eightNeighbors.none { it in elves }) return current

      val numRules = values().size
      val rules: List<ElfMovementRule> = (roundNumber until roundNumber + numRules)
        .map { it % numRules }
        .map { values()[it] }

      val delta = rules
        .firstOrNull { rule ->
          rule.consideredPositions
            .map { it + current }
            .none { it in elves }
        }
        ?.direction?.toPoint()
...
private object AdjacentPoint {
  val N = Direction.N.toPoint()
  val NE = Direction.N.toPoint() + Direction.E.toPoint()
  val E = Direction.E.toPoint()
  val SE = Direction.S.toPoint() + Direction.E.toPoint()
  val S = Direction.S.toPoint()
  val SW = Direction.S.toPoint() + Direction.W.toPoint()
  val W = Direction.W.toPoint()
  val NW = Direction.N.toPoint() + Direction.W.toPoint()
}

fun main() {
  val input = File("src/main/kotlin/advent/year2022/day23/input.txt").readText()
    .trimEnd()

  val elves = PlantingElves(input)

  println(elves.after(10).emptyGround())
  println(elves.firstNoMoveRound())
}