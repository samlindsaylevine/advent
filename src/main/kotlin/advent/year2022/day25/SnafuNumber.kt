 ...
 * correctly calibrate heat output and flow rate. This amount is simply the sum of the fuel requirements of all of the
 * hot air balloons, and those fuel requirements are even listed clearly on the side of each hot air balloon's burner.
 ...
 * are 2, 1, 0, minus (written -), and double-minus (written =). Minus is worth -1, and double-minus is worth -2."
 ...
 * (double-minus) in the 125s place, - (minus) in the 25s place, 0 in the 5s place, and 1 in the 1s place. (2 times
 * 625) plus (-2 times 125) plus (-1 times 25) plus (0 times 5) plus (1 times 1). That's 1250 plus -250 plus -25 plus 0
 * plus 1. 976!"
...
fun String.parseSnafu(): Long {
  ...
  val digit = when (val last = this.last()) {
    '1' -> 1
    '2' -> 2
    '0' -> 0
    '-' -> -1
    '=' -> -2
    else -> throw IllegalArgumentException("Unparseable digit $last")
  }
  return digit + 5 * this.dropLast(1).parseSnafu()
}

fun Long.toSnafu(): String {
  if (this == 0L) return ""
  val modulo = this % 5
  val onesPlace = if (modulo > 2) modulo - 5 else modulo
  val digit = when (onesPlace) {
    1L -> '1'
    2L -> '2'
    0L -> 0
    -1L -> '-'
    -2L -> '='
    else -> throw IllegalStateException("Invalid modulo result $onesPlace while converting $this, this is a bug.")
  }
  return ((this - onesPlace) / 5).toSnafu() + digit
}

fun main() {
  val input = File("src/main/kotlin/advent/year2022/day25/input.txt").readText().trimEnd()
