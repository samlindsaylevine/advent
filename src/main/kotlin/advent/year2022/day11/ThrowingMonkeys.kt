...
class ThrowingMonkeys(
  ...
  val modulusProduct = monkeys.map { it.modulus }.reduce(Long::times)
}
...
class ThrowingMonkey(
  ...
  fun next(monkeys: ThrowingMonkeys) {
    ...
    items.forEach { item ->
      ...
      // The relaxed part of this is obvious - just divide by 3. The non-relaxed mode is less obvious.
      // Each monkey is operating according to modular arithmetic - in its own Z/whatever. A helpful hint is that the
      // monkey moduluses are all co-prime - in fact, in my input, they are all prime numbers.
      // That means that we can always operate with our worry numbers reduced mod (product of all moduluses) - it will
      // always behave the same in the eyes of each individual monkey.
      val postBoredom = if (relaxed) postInspect / 3 else postInspect % monkeys.modulusProduct
...
fun main() {
  ...
  val worriedMonkeys = ThrowingMonkeys(input, relaxed = false)
  worriedMonkeys.next(10_000)
  println(worriedMonkeys.monkeyBusiness())
}