 ...
 * able to do so, at each step trying to move down, then down-left, then down-right. If all three possible destinations
...
class SandyCave(
  ...
  val sand: Set<Point> = emptySet()
) {
  constructor(input: String, hasFloor: Boolean = false) : this(
    rocks = input.lines().flatMap { line ->
      line.split(" -> ")
        .zipWithNext()
        .flatMap { it.first.parseAsPoint()..it.second.parseAsPoint() }
    }.toSet(),
    hasFloor = hasFloor
  )

  companion object {
    private fun String.parseAsPoint() = Point(
      this.substringBefore(",").toInt(),
      this.substringAfter(",").toInt()
    )

    private val sandEntryPoint = Point(500, 0)
  }

  private val occupied = rocks + sand
  private val maxY = rocks.maxOf { it.y }
  private fun available(point: Point) = when {
    occupied.contains(point) -> false
    hasFloor && point.y >= maxY + 2 -> false
    else -> true
  }

  fun filled(): SandyCave = generateSequence(this) {
    val next = it.nextSand()
    if (next == null) null else SandyCave(it.rocks, it.hasFloor, it.sand + next)
  }.last()

  // The place the next unit of falling sand will come to rest, if any.
  private fun nextSand(): Point? {
    val last = fallingSand().last()
    // If we have no floor, we're done when we are into the void; we're always done if the new sand point is already in
    // our existing set of sand (i.e., we are clogged).
    return if (last.hasReachedTheVoid() || last in this.sand) null else last
  }

  private fun Point.hasReachedTheVoid() = !hasFloor && this.y > maxY

  // Returns all the positions of the next unit of falling sand.
  private fun fallingSand(): Sequence<Point> = generateSequence(sandEntryPoint) {
    when {
      it.hasReachedTheVoid() -> null
      available(it + Point(0, 1)) -> it + Point(0, 1)
      available(it + Point(-1, 1)) -> it + Point(-1, 1)
      available(it + Point(1, 1)) -> it + Point(1, 1)
      else -> null
    }
  }
...
fun main() {
  val input = File("src/main/kotlin/advent/year2022/day14/input.txt").readText().trim()

  val cave = SandyCave(input)
  println(cave.filled().sand.size)

  val withFloor = SandyCave(input, hasFloor = true)
  println(withFloor.filled().sand.size)
}
