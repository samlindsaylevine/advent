 ...
 *
 */
typealias CleanupAssignments = List<Pair<IntRange, IntRange>>

fun String.asCleanupAssignments(): CleanupAssignments = this.lines()
  .map { it.substringBefore(",").parseAsIntRange() to it.substringAfter(",").parseAsIntRange() }

private fun String.parseAsIntRange(): IntRange = (this.substringBefore("-").toInt()..this.substringAfter("-").toInt())

private operator fun IntRange.contains(other: IntRange) = this.first <= other.first && this.last >= other.last

fun CleanupAssignments.fullyContainedCount() = this.count { it.first in it.second || it.second in it.first }
fun CleanupAssignments.overlapCount() = this.count { (it.first intersect it.second).isNotEmpty() }

fun main() {
  val input = File("src/main/kotlin/advent/year2022/day4/input.txt").readText().trim()
  val assignments = input.asCleanupAssignments()

  println(assignments.fullyContainedCount())
  println(assignments.overlapCount())
}
