 ...
 *
 * During the 20th cycle, register X has the value 21, so the signal strength is 20 * 21 = 420. (The 20th cycle occurs
 * in the middle of the second addx -1, so the value of register X is the starting value, 1, plus all of the other addx
...
class ClockCircuit(
  ...
  val xRegister: Int = 1,
  private val cyclesSpentOnCurrentInstruction: Int = 0,
  private val remainingInstructions: List<ClockCircuitInstruction>
) {
  constructor(input: String) : this(remainingInstructions = input.lines().map(ClockCircuitInstruction::parse))

  fun next(): ClockCircuit? {
    val instruction = remainingInstructions.firstOrNull() ?: return null
    return when (instruction) {
      is NoOp -> ClockCircuit(cycleNumber + 1, xRegister, 0, remainingInstructions.drop(1))
      is AddX -> if (cyclesSpentOnCurrentInstruction == 0) {
        ClockCircuit(cycleNumber + 1, xRegister, 1, remainingInstructions)
      } else {
        ClockCircuit(cycleNumber + 1, xRegister + instruction.amount, 0, remainingInstructions.drop(1))
      }
    }
  }

  fun execution(): Sequence<ClockCircuit> = generateSequence(this, ClockCircuit::next)

  fun interestingSignalStrength() = execution()
    .filter { it.cycleNumber % 40 == 20 }
    .filter { it.cycleNumber <= 220 }
    .map { it.cycleNumber * it.xRegister }
    .sum()

  fun draw(): String = execution()
    .take(240)
    .map {
      val x = (it.cycleNumber - 1) % 40
      if ((x - it.xRegister).absoluteValue <= 1) '#' else '.'
    }.chunked(40)
    .joinToString("\n") { it.joinToString("") }
}

sealed class ClockCircuitInstruction {
  companion object {
    fun parse(input: String): ClockCircuitInstruction = when (input) {
      "noop" -> NoOp
      else -> AddX(input.substringAfter(" ").toInt())
    }
  }
}

private object NoOp : ClockCircuitInstruction()
private data class AddX(val amount: Int) : ClockCircuitInstruction()

fun main() {
  val input = File("src/main/kotlin/advent/year2022/day10/input.txt").readText().trim()
