 ...
 *
 */
data class FoodCarryingElf(val calories: List<Int>) {
  val totalCalories = calories.sum()
}

fun parseElves(input: String): List<FoodCarryingElf> = input.split("\n\n")
  .map(::parseElf)

private fun parseElf(elfInput: String) = elfInput.lines()
  .map(String::toInt)
  .let(::FoodCarryingElf)

fun List<FoodCarryingElf>.caloriesFromTopElves(numElves: Int) = this.map { it.totalCalories }
  .sortedDescending()
  .take(numElves)
  .sum()

fun List<FoodCarryingElf>.mostCalories() = this.caloriesFromTopElves(1)

fun main() {
  val input = File("src/main/kotlin/advent/year2022/day1/input.txt").readText().trim()
  val elves = parseElves(input)
  println(elves.mostCalories())
  println(elves.caloriesFromTopElves(numElves = 3))
}