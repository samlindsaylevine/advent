 ...
 * it only visits 1 position. However, be careful: more types of motion are possible than before, so you might want to
...
class RopePath(
  ...
  val tailVisited: Set<Point>
) {
  constructor(numKnots: Int) : this(List(numKnots) { Point(0, 0) }, setOf(Point(0, 0)))
  ...
  private fun move(direction: Direction): RopePath {
    val nextKnots = mutableListOf(knots.first() + direction.toPoint())
    (1 until knots.size).forEach { index ->
      nextKnots.add(follow(nextKnots[index - 1], knots[index]))
    }
    return RopePath(nextKnots, this.tailVisited + nextKnots.last())
  }

  private fun follow(target: Point, follower: Point): Point {
...
fun main() {
  val input = File("src/main/kotlin/advent/year2022/day9/input.txt").readText().trim().lines()

  val smallRopePath = RopePath(2).move(input)
  println(smallRopePath.tailVisited.size)

  val largeRopePath = RopePath(10).move(input)
  println(largeRopePath.tailVisited.size)
}