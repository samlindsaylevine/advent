...
class GardenMap(val start: Point,
  ...
  // fast enough.
  data class GardenTraversal(val thisParityInteriorCount: Int,
  ...
  fun reachable(): Sequence<GardenTraversal> = generateSequence(GardenTraversal(0, 0, emptySet(), setOf(start))) { next(it) }
  ...
  fun countByRecurrence(numSteps: Int, loadDataFromFile: Boolean = false): Long {
    ...
    val quadratic = Quadratic.fit(datapoint(appropriateStart), datapoint(appropriateStart + period), datapoint(appropriateStart + 2 * period))
...
data class Quadratic(val a: Rational, val b: Rational, val c: Rational) {
  ...
  constructor(a: Long, b: Long, c: Long) : this(Rational.of(a, 1),
          Rational.of(b, 1),
          Rational.of(c, 1))
