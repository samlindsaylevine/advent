 ...
 *
 */
data class ColoredCubeGames(val games: List<ColoredCubeGame>) {
    companion object {
        val partOneColors = mapOf("red" to 12, "green" to 13, "blue" to 14)
    }

    constructor(input: String): this(input.trim().lines().map(::ColoredCubeGame))

    fun possibleIdSum(countsByColor: Map<String, Int>) = games
            .filter { it.isPossible(countsByColor) }
            .sumOf { it.id }

    fun totalPower() = games.sumOf { it.power() }
}

data class ColoredCubeGame(val id: Int,
        val cubeShowoffs: List<CubeShowoff>) {
    constructor(input: String): this(
            input.substringBetween("Game ", ":").toInt(),
            input.substringAfter(": ").split("; ").map(::CubeShowoff)
    )

    private fun shownCubesOfColor(color: String) = cubeShowoffs.maxOf { it.cubesByColor[color] ?: 0 }
    fun isPossible(countsByColor: Map<String, Int>) = countsByColor.entries
            .all { (color, count) -> this.shownCubesOfColor(color) <= count }

    fun power() = shownCubesOfColor("red") * shownCubesOfColor("green") * shownCubesOfColor("blue")
}

data class CubeShowoff(val cubesByColor: Map<String, Int>) {
    constructor(input: String) : this(input.split(", ")
            .associate { it.substringAfter(" ") to it.substringBefore(" ").toInt() })
}

fun main() {
    val input = File("src/main/kotlin/advent/year2023/day2/input.txt").readText().trim()
    val games = ColoredCubeGames(input)

    println(games.possibleIdSum(ColoredCubeGames.partOneColors))
    println(games.totalPower())
}