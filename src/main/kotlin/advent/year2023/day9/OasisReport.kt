 ...
 *
 */
class OasisReport(val sequences: List<VaryingOasisSequence>) {
  constructor(input: String) : this(input.trim().lines().map(::VaryingOasisSequence))

  fun sumOfNext() = sequences.sumOf { it.next() }
  fun sumOfPrevious() = sequences.sumOf { it.previous() }
}

sealed interface OasisSequence {
  fun get(index: Int): Int
}

data class ConstantOasisSequence(val value: Int) : OasisSequence {
  override fun get(index: Int) = value
}

class VaryingOasisSequence(private val knownValues: List<Int>) : OasisSequence {
  constructor(input: String) : this(input.findAllNumbers())

  // These are all polynomials, since there are a fixed number of child sequences until we
  // get to a fixed sequence. We could do the math to do the regression and find the formula
  // and just evaluate it. Instead, I'm going to use the recurrence relation, and a cache
  // to memoize values.
  override fun get(index: Int): Int = valueByIndexCache[index]

  fun next() = get(knownValues.size)
  fun previous() = get(-1)

  private val valueByIndexCache = loadingCache<Int, Int> { index ->
    when {
      index < 0 -> get(index + 1) - childSequence.get(index)
      index < knownValues.size -> knownValues[index]
      else -> get(index - 1) + childSequence.get(index - 1)
    }
  }

  private val childSequence: OasisSequence by lazy {
    val diffs = knownValues.zipWithNext { first, second -> second - first }
    if (diffs.all { it == diffs.first() }) ConstantOasisSequence(diffs.first()) else VaryingOasisSequence(diffs)
  }
}

fun main() {
  val input = File("src/main/kotlin/advent/year2023/day9/input.txt").readText().trim()
  val report = OasisReport(input)

  println(report.sumOfNext())
  println(report.sumOfPrevious())
}