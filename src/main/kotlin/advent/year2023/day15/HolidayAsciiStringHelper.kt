...
fun String.hashed() = this.fold(0) { acc, c -> (acc + c.code) * 17 % 256 }

class InitializationSequence(val steps: List<String>) {
  constructor(input: String) : this(input.split(","))

  fun result(): List<Box> {
    val boxes = List(256) { Box() }
    steps.map(InitializationStep::of).forEach { it.apply(boxes) }
    return boxes
  }
}

data class Box(val lenses: MutableList<Lens> = mutableListOf()) {
  fun focusingPower() = lenses.mapIndexed { i, lens -> (i + 1) * lens.focalLength }.sum()
}

fun List<Box>.focusingPower() = this.mapIndexed { i, box -> (i + 1) * box.focusingPower() }.sum()

data class Lens(val label: String, val focalLength: Int)

sealed interface InitializationStep {
  companion object {
    fun of(input: String) = when {
      input.endsWith("-") -> RemoveStep(input.substringBefore("-"))
      else -> UpdateStep(input.substringBefore("="), input.substringAfter("=").toInt())
    }
  }

  val label: String

  fun apply(boxes: List<Box>) = this.apply(boxes[label.hashed()])

  fun apply(box: Box)
}

data class RemoveStep(override val label: String) : InitializationStep {
  override fun apply(box: Box) {
    box.lenses.removeIf { it.label == this.label }
  }
}

data class UpdateStep(override val label: String, val focalLength: Int) : InitializationStep {
  override fun apply(box: Box) {
    val newLens = Lens(this.label, this.focalLength)
    val existingLensIndex = box.lenses.indexOfFirst { it.label == this.label }
    if (existingLensIndex == -1) box.lenses.add(newLens) else box.lenses[existingLensIndex] = newLens
  }
}


fun main() {
  val input = File("src/main/kotlin/advent/year2023/day15/input.txt").readText().trim()
  val sequence = InitializationSequence(input)

  println(sequence.steps.sumOf { it.hashed() })
  println(sequence.result().focusingPower())
}