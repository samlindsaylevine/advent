 ...
 *
 */
class ObservedCosmos(private val galaxies: Set<Point>) {
  ...
  fun expanded(amount: Int = 2): ObservedCosmos {
    val xValues = galaxies.map { it.x }.toSet()
    val yValues = galaxies.map { it.y }.toSet()
    val expandingXValues = (galaxies.minOf { it.x }..galaxies.maxOf { it.x }).filter { it !in xValues }
    val expandingYValues = (galaxies.minOf { it.y }..galaxies.maxOf { it.y }).filter { it !in yValues }
    val newGalaxies = galaxies.map { (originalX, originalY) ->
      val newX = originalX + expandingXValues.count { originalX > it } * (amount - 1)
      val newY = originalY + expandingYValues.count { originalY > it } * (amount - 1)
      Point(newX, newY)
    }
    return ObservedCosmos(newGalaxies.toSet())
  }

  fun sumOfLengths() = galaxies.toList().allUnorderedPairs().sumOf { (first, second) ->
    first.distanceFrom(second).toLong()
  }
}

fun main() {
  val input = File("src/main/kotlin/advent/year2023/day11/input.txt").readText().trim()
  val cosmos = ObservedCosmos(input)

  println(cosmos.expanded().sumOfLengths())
  println(cosmos.expanded(1_000_000).sumOfLengths())
}