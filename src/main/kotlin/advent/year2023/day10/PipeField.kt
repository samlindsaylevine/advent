...
class PipeField(val tiles: Map<Point, Char>) {
  ...
  // Not including the start, but including the end space.
  private fun animalLoop() = animalPath().drop(1).takeWhileInclusive { tiles[it.current] != 'S' }

  fun stepsToFarthest(): Int {
    val pathLength = animalLoop().size()
    return ceil((pathLength.toFloat()) / 2).toInt()
  }

  fun enclosedTileCount(): Int {
    // Including both the start point as both the start and end of the list.
    val totalPath: List<Point> = listOf(start()) + animalLoop().map { it.current }

    // The most straightforward way in my mind to calculate which spaces are inside the loop is to "flood fill"; i.e.,
    // start with a point just outside the loop, and then recursively walk to its neighbors and mark those neighbors
    // as also "outside", being unwilling to cross the spaces with the loop.
    // However, this has a problem that it doesn't handle the "squeak through" situation where we should be able to
    // flood into an area that has two adjacent pipes: "|"
    // After thinking about this for a while, and toying with flooding "0.5" pseudo-spaces, I realized that we could
    // do that more cleanly by transforming the coordinates into a new space where we double all the original
    // coordinates. Then we fill in the loop walls at the new odd-coordinated points, and can again flood fill,
    // finally selecting out only those enclosed points that have both even coordinates.
    val expandedLoop = totalPath.map { 2 * it }
            .zipWithNext()
            .flatMap { (first, second) -> listOf(first, (first + second) / 2) }
            .toSet()

    val minX = expandedLoop.minOf { it.x } - 1
    val maxX = expandedLoop.maxOf { it.x } + 1
    val minY = expandedLoop.minOf { it.y } - 1
    val maxY = expandedLoop.maxOf { it.y } + 1

    val findableFromOutside = floodFill(Point(minX, minY)) { point ->
      point.x in minX..maxX && point.y in minY..maxY && point !in expandedLoop
    }
...
fun main() {
  ...
  println(field.enclosedTileCount())
}