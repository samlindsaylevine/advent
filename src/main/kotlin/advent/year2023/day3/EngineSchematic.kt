 ...
 *
 */
class EngineSchematic(private val numbers: Set<SchematicNumber>, private val symbols: Set<SchematicSymbol>) {
    companion object {
        fun of(input: String): EngineSchematic = input.trim()
                .lines()
                .mapIndexed { y, line -> ofLine(y, line) }
                .reduce(EngineSchematic::plus)

        private fun ofLine(y: Int, line: String): EngineSchematic {
            val regex = Regex("\\d+")
            val allMatches = regex.findAll(line)
            val numbers = allMatches.map { match -> SchematicNumber(Point(match.range.first, y), match.value.toInt()) }
                    .toSet()
            val symbols = line.withIndex()
                    .filter { !it.value.isDigit() && it.value != '.' }
                    .map { SchematicSymbol(Point(it.index, y), it.value) }
                    .toSet()
            return EngineSchematic(numbers, symbols)
        }
    }

    operator fun plus(other: EngineSchematic) = EngineSchematic(this.numbers + other.numbers, this.symbols + other.symbols)

    private val symbolPositions = symbols.map { it.position }.toSet()
    private val partNumbers = numbers.filter { (it.adjacentPositions() intersect symbolPositions).isNotEmpty() }
    fun partNumberSum() = partNumbers.sumOf { it.value }

    private fun adjacentNumbers(point: Point) = numbers.filter { point in it.adjacentPositions() }
    fun gearRatioSum() = symbols.asSequence()
            .filter { it.symbol == '*' }
            .map { adjacentNumbers(it.position) }
            .filter { it.size == 2 }
            .map { it.map { number -> number.value }.reduce(Int::times) }
            .sum()
}

data class SchematicNumber(val leftmostPosition: Point, val value: Int) {
    fun adjacentPositions(): Set<Point> {
        val length = floor(log10(this.value.toDouble())).toInt() + 1
        val occupiedPositions = (0 until length).map { Point(it, 0) + leftmostPosition }
        return occupiedPositions.flatMap { it.eightNeighbors }.toSet() - occupiedPositions.toSet()
    }
}

data class SchematicSymbol(val position: Point, val symbol: Char)

fun main() {
    val input = File("src/main/kotlin/advent/year2023/day3/input.txt").readText().trim()
    val schematic = EngineSchematic.of(input)

    println(schematic.partNumberSum())
    println(schematic.gearRatioSum())
}