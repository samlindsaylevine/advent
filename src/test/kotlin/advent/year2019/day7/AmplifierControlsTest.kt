...
class AmplifierControlsTest {
    ...
    @Test
    fun `optimalLoopedSettings -- first reference example -- reference values`() {
        val program = listOf(3, 26, 1001, 26, -4, 26, 3, 27, 1002, 27, 2, 27, 1, 27, 26,
                27, 4, 27, 1001, 28, -1, 28, 1005, 28, 6, 99, 0, 0, 5)
        val amplifierControls = AmplifierControls(program = program)

        val optimal = amplifierControls.optimalLoopedSettings()

        assertThat(optimal.settings).containsExactly(9, 8, 7, 6, 5)
        assertThat(optimal.thrusterSignal).isEqualTo(139629729)
    }

    @Test
    fun `optimalLoopedSettings -- second reference example -- reference values`() {
        ...
        assertThat(optimal.settings).containsExactly(9, 7, 8, 5, 6)
        assertThat(optimal.thrusterSignal).isEqualTo(18216)
    }
}