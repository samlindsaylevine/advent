...
class BatteryBanksTest {
  ...
  fun `largest joltage -- reference examples -- reference values`(input: String, expected: Int) {
    val batteryBank = BatteryBank(input)

    val joltage = batteryBank.largestJoltage()
  ...
  fun `total output joltage -- reference example -- 357`() {
    ...
    assertThat(joltage).isEqualTo(357)
  }
}